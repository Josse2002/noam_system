...
function TableCart() {
    ...
    const deleteItemInDb = async (position) => {
        ...
        try {
            await updateDoc(cartRef, {
                cart: updatedCart.map((item) => item.productId),
                cartNumbers: updatedCart.map((item) => item.quantity),
            });
        } catch (error) {
            console.error("Error al actualizar carrito en la base de datos:", error);
        }
    ...
    const deleteItem = (productId, position) => {
        dispatch(removeCart(productId, position));
                ...
                <tbody>
                    {
                        items.map((item, index) => (
                            ...
                            <tr className='product-cart-line' key={index}>
                                ...
                                <td><button className='delete-button' onClick={() => deleteItem(item.productId, index)}><i class="fa-sharp fa-solid fa-xmark"></i></button></td>
                            </tr>
                        ))
                    }
                </tbody>
